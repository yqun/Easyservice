<template>
  <div>
    ...
    <group>
      <x-input :class="{border: !f_description}" title="　　　问题：" :show-clear="false" v-model="f_description"></x-input>
      ...
      <x-input :class="{border: !address}" title="　服务地址：" disabled v-model="address">
        <x-button slot="right" mini :gradients="btncolor" :link="`/oftenaddress?id=${orderId}`">选择</x-button>
      </x-input>
      <x-input :class="{border: !f_address}" title="　详细地址：" :show-clear="false" v-model="f_address"></x-input>
